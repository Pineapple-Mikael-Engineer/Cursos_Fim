...
> [!info]
> Cierra la maquinaria de cálculo del bloque [[Sistemas y Dinamica/index| sistemas y dinámica]]. Es la [[Matriz Fundamental| matriz fundamental]] **normalizada** con $\Phi(0)=I$, y la pieza que la [[Variacion de Parametros Sistemas| variación de parámetros]] usa en la fórmula de Duhamel. Se calcula con los [[Sistemas Lineales Autovalores| autovalores]] de $A$.
...
> **Calcular $e^{At}$ para $A=\begin{pmatrix}1&2\\2&1\end{pmatrix}$.** Autovalores $\lambda=3,-1$ con autovectores $\mathbf{v}_1=(1,1),\mathbf{v}_2=(1,-1)$, así $A=PDP^{-1}$ con $P=\begin{pmatrix}1&1\\1&-1\end{pmatrix}$, $D=\operatorname{diag}(3,-1)$, $P^{-1}=\tfrac12\begin{pmatrix}1&1\\1&-1\end{pmatrix}$. Entonces
...
> **Calcular $e^{At}$ para $A=\begin{pmatrix}2&1\\0&2\end{pmatrix}$.** Se escribe $A=2I+N$ con $N=\begin{pmatrix}0&1\\0&0\end{pmatrix}$ **nilpotente** ($N^2=0$). Como $2I$ y $N$ **conmutan**,
> $$e^{At}=e^{2It}\,e^{Nt}=e^{2t}\,(I+Nt)=e^{2t}\begin{pmatrix}1&t\\0&1\end{pmatrix}.$$
> El factor $t$ es la huella del autovalor **repetido deficiente** (mismo origen que en [[Sistemas Lineales Autovalores| el autovector generalizado]]). La serie se **corta** porque $N$ es nilpotente: $e^{Nt}=I+Nt$ exacto.
...
> La función $t\mapsto e^{At}$ cumple $\dfrac{d}{dt}e^{At}=A\,e^{At}$ y $e^{A\cdot0}=I$. Por tanto $\mathbf{x}(t)=e^{At}\mathbf{x}_0$ es la **única** solución de $\dot{\mathbf{x}}=A\mathbf{x}$ con $\mathbf{x}(0)=\mathbf{x}_0$.
...
> [!warning]
> En general $e^{A+B}\neq e^Ae^B$ **si $A$ y $B$ no conmutan**. Por eso no se puede "separar" la exponencial de una suma de matrices arbitrarias.

> [!algoritmo] Calcular $e^{At}$
> 1. **Diagonalizable** ($n$ autovectores): $e^{At}=P\,\operatorname{diag}(e^{\lambda_it})\,P^{-1}$.
> 2. **Deficiente**: descompón $A=S+N$ (semisimple + nilpotente que conmutan, forma de Jordan) y usa $e^{At}=e^{St}e^{Nt}$ con $e^{Nt}=\sum_{j<m}\frac{(Nt)^j}{j!}$ (serie finita).
...
> [!corolario]
> $e^{At}$ comprime toda la dinámica lineal en un solo objeto: **propaga** el estado inicial al instante $t$. Sus autovalores ($e^{\lambda_it}$) reproducen modo a modo el crecimiento, el decaimiento y la rotación que predicen los [[Sistemas Lineales Autovalores| autovalores]] de $A$.
