...
{\footnotesize Esquema $x^{(k+1)}=G(x^{(k)})$ para un sistema no lineal:
\[
x'=g_1=\frac{x+\cos y}{4},\qquad
y'=g_2=\frac{y+e^{-x}}{5}.
\]
Analizar la convergencia en $x^{(0)}=0,\ y^{(0)}=0$ con el \textbf{Teorema del punto fijo}.}
\end{cardidea}
\vspace{1mm}
\begin{cardnota}[\faInfoCircle]
{\footnotesize La norma $\infty$ de una matriz es el \emph{máximo} de las sumas de valores absolutos por fila:
...
\]
\textbf{3.} Norma $\infty$ (máx.\ suma de filas):
\[
L=\max\big(\underbrace{0.25}_{|0.25|+0},\ \underbrace{0.4}_{|{-}0.2|+|0.2|}\big)=0.4.
\]}
\end{cardgear}
\vspace{1mm}
...
\begin{cardidea}{Enunciado}{\faQuestionCircle}
{\footnotesize Trayectoria \emph{suave} que parte de $(0,0)$, pasa por los puntos de control $(2,4)$ y $(6,4)$, recoge en $(7,6)$, pasa por $(12,7)$ y deja la pieza en $(15,1)$. Escribir un script que la grafique con \texttt{spline} de MATLAB.}
\end{cardidea}
\vspace{1mm}
...
\begin{cardidea}{Enunciado}{\faQuestionCircle}
{\footnotesize Con \textbf{3 puntos} se aproxima una integral suave por tres métodos:
Simpson \emph{cerrado}, Simpson \emph{abierto} y \emph{Gauss--Legendre}.
Ordenarlos de \emph{menor} a \emph{mayor} precisión esperada y justificar.}
\end{cardidea}
\vspace{1mm}
...
\begin{cardidea}{Enunciado}{\faQuestionCircle}
{\footnotesize EDO $y'=3t-2y$ con $y(0)=0.5$ en $t\in[0,2]$. Rellenar los \textbf{4 espacios en blanco} del script funcional (resaltados).}
\end{cardidea}
\vspace{1mm}
\begin{columns}[T]
...
% ---- P1 enunciado ----
\begin{frame}{Problema 1 — Planteamiento}
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\begin{cardidea}{Enunciado}{\faQuestionCircle}
{\footnotesize Brazo plano de 2 eslabones ($L_1=30$ cm, $L_2=20$ cm). El efector final es
\[
x=L_1\cos\theta_1+L_2\cos(\theta_1+\theta_2),\quad
y=L_1\sin\theta_1+L_2\sin(\theta_1+\theta_2).
\]
Configurar el robot para alcanzar el objetivo $(40,20)$ cm, con incógnitas $\mathbf{x}=[\theta_1,\theta_2]^{\!\top}$.}
\end{cardidea}
\vspace{1mm}
\begin{cardnota}[\faTasks]
{\footnotesize \textbf{Se pide:} (a) plantear $F(\mathbf x)=0$ y su Jacobiano $J_F$; (b) 2 iteraciones de Newton-Raphson; (c) demostrar $\det J=L_1L_2\sin\theta_2$ y analizar la singularidad.}
\end{cardnota}
\end{column}
\begin{column}{0.43\textwidth}
\begin{cardgear}{Sistema no lineal $F(\mathbf x)=0$}{\faSuperscript}
{\footnotesize Se pasa el objetivo al lado izquierdo:
\[
f_1=30\cos\theta_1+20\cos(\theta_1+\theta_2)-40=0,
\]
\[
f_2=30\sin\theta_1+20\sin(\theta_1+\theta_2)-20=0.
\]}
\end{cardgear}
\vspace{1mm}
\begin{cardnota}[\faInfoCircle]
{\footnotesize Dos ecuaciones, dos incógnitas: sistema no lineal resoluble por linealización iterada (Newton-Raphson).}
\end{cardnota}
\end{column}
\end{columns}
\end{frame}
...
% ---- P2 enunciado ----
\begin{frame}{Problema 2 — Planteamiento}
\begin{columns}[T]
\begin{column}{0.53\textwidth}
\begin{cardidea}{Enunciado}{\faQuestionCircle}
{\footnotesize Núcleo ferromagnético modelado por Fröhlich-Kennelly:
\[
B(H)=\frac{a\sqrt H}{1+b\sqrt H}\quad[\text{T}],\ H\ \text{en kA/m}.
\]
...
\begin{cardnota}[\faTasks]
{\footnotesize \textbf{Se pide:}
(a) ajuste por mínimos cuadrados $\to a,b$;
(b) coeficiente $R^2$;
(c) estimar $B(6.25)$;
(d) interpretar físicamente $a/b$ cuando $H\to\infty$.}
\end{cardnota}
\vspace{1mm}
\begin{cardidea}{Idea del ajuste lineal}{\faLightbulb}
{\footnotesize El modelo no es lineal en $a,b$. Se \emph{linealiza} tomando el recíproco de $B$ para poder aplicar mínimos cuadrados ordinarios.}
\end{cardidea}
\end{column}
\end{columns}
\end{frame}
...
% ---- P3 enunciado ----
\begin{frame}{Problema 3 — Planteamiento}
\begin{columns}[T]
\begin{column}{0.53\textwidth}
\begin{cardidea}{Enunciado}{\faQuestionCircle}
{\footnotesize Compuerta en forma de cuarto de círculo (radio $R$). Presión $p(y)=\rho g y$; ancho $b(y)=\sqrt{R^2-y^2}$. Fuerza total:
\[
F=\int_0^R p(y)\,b(y)\,dy=\int_0^R \rho g\,y\sqrt{R^2-y^2}\,dy.
\]
Datos: $\rho=1000\ \text{kg/m}^3$, $g=9.81\ \text{m/s}^2$, $R=2$ m.}
\end{cardidea}
\end{column}
\begin{column}{0.45\textwidth}
\begin{cardnota}[\faTasks]
{\footnotesize \textbf{Se pide:}
(a) $F$ por Simpson $1/3$ con $n=4$;
(b) $F$ por Gauss 3 puntos;
(c) error relativo de cada uno frente a $F_{\text{exacta}}=\rho gR^3/3$ y decidir el más eficiente.}
\end{cardnota}
\vspace{1mm}
\begin{cardgear}{Integrando}{\faSuperscript}
{\footnotesize
\[
f(y)=\rho g\,y\sqrt{R^2-y^2}.
\]
Se evaluará en los nodos de cada regla.}
\end{cardgear}
\end{column}
\end{columns}
\end{frame}
