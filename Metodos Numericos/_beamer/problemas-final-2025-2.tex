...
\[
x^{(k+1)}=g_1(x^{(k)},y^{(k)})=\frac{x^{(k)}+\cos y^{(k)}}{4},
\]
\[
y^{(k+1)}=g_2(x^{(k)},y^{(k)})=\frac{y^{(k)}+e^{-x^{(k)}}}{5}.
\]
...
\vspace{1mm}
\begin{cardnota}[\faInfoCircle]
{\footnotesize \textbf{Criterio:} $\lVert A\rVert_\infty=\max_i\sum_j|a_{ij}|$ (máx.\ suma por fila). Si $\lVert J_G\rVert_\infty<1$, $G$ es contracción.}
\end{cardnota}
\end{column}
...
\[
J_G(x,y)=\begin{pmatrix} 0.25 & -0.25\sin y\\ -0.2\,e^{-x} & 0.2 \end{pmatrix}.
\]
\textbf{2.} Evaluar en $(0,0)$ ($\sin 0=0,\ e^{0}=1$):
\[
J_G(0,0)=\begin{pmatrix} 0.25 & 0\\ -0.2 & 0.2 \end{pmatrix}.
\]
\textbf{3.} Sumas por fila: $0.25+0=0.25$ y $|{-}0.2|+|0.2|=0.4$, así $L=\max(0.25,\,0.4)=0.4$.

\smallskip
\textbf{4.} Como $L=0.4<1$, $G$ es \textbf{contracción}: el método \emph{sí} converge (linealmente).}
\end{cardgear}
\end{column}
\end{columns}
\end{frame}
